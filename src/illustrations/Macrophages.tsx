...
export function Macrophages({ className = "" }: { className?: string }) {
  ...
  // Constants for the shared "Target" area (bottom-right)
  const BASE_DX = 1700;
  const BASE_DY = 700;
  ...
  // m = Multiplier to maintain varied slopes based on (BASE_DX, BASE_DY)
  const particles = [
    { cx: 200, cy: 165, m: 1.04, dur: 9.2, delay: 0.0 },
    { cx: 150, cy: 195, m: 1.00, dur: 9.6, delay: 0.4 },
    { cx: 100, cy: 235, m: 1.05, dur: 8.8, delay: 0.8 },
    { cx: 250, cy: 185, m: 0.97, dur: 9.2, delay: 1.2 },
    { cx: 180, cy: 255, m: 1.01, dur: 9.5, delay: 1.6 },
    { cx: 120, cy: 295, m: 1.03, dur: 8.9, delay: 2.0 },
    { cx: 230, cy: 235, m: 0.94, dur: 9.3, delay: 2.4 },
    { cx:  80, cy: 335, m: 1.07, dur: 9.6, delay: 0.6 },
    { cx: 160, cy: 315, m: 1.00, dur: 9.0, delay: 1.0 },
    { cx: 270, cy: 265, m: 0.91, dur: 9.4, delay: 1.4 },
    { cx:  50, cy: 365, m: 1.10, dur: 9.7, delay: 1.8 },
    { cx: 190, cy: 145, m: 1.01, dur: 9.0, delay: 2.2 },
    { cx: 270, cy: 295, m: 0.90, dur: 9.5, delay: 2.6 },
    { cx:  70, cy: 265, m: 1.09, dur: 9.8, delay: 3.0 },
    { cx: 220, cy: 345, m: 0.93, dur: 9.3, delay: 0.2 },
    { cx: 120, cy: 365, m: 1.04, dur: 8.7, delay: 1.5 },
    // Extra particles for a denser stream toward the bottom-right corner.
    { cx: 220, cy: 205, m: 0.98, dur: 9.1, delay: 0.3 },
    { cx:  95, cy: 295, m: 1.07, dur: 9.5, delay: 0.7 },
    { cx: 175, cy: 325, m: 1.00, dur: 8.8, delay: 1.1 },
    { cx: 245, cy: 225, m: 0.96, dur: 9.4, delay: 1.5 },
    { cx:  60, cy: 220, m: 1.09, dur: 9.7, delay: 1.9 },
    { cx: 290, cy: 305, m: 0.91, dur: 9.0, delay: 2.3 },
    { cx: 140, cy: 405, m: 1.02, dur: 8.9, delay: 2.7 },
    { cx:  30, cy: 315, m: 1.12, dur: 9.6, delay: 0.5 },
    { cx: 210, cy: 385, m: 0.93, dur: 9.2, delay: 1.3 },
    { cx: 100, cy: 180, m: 1.06, dur: 9.4, delay: 1.7 },
    { cx: 260, cy: 155, m: 0.96, dur: 9.0, delay: 2.1 },
    { cx:  40, cy: 405, m: 1.11, dur: 9.5, delay: 2.5 },
  ];
  ...
  return (
    ...
    >
      ...
      <g className="mc-cell mc-cell-1">
        <path fill={`url(#${gBody})`} stroke="#7BC9E8" strokeOpacity="0.22" strokeWidth="1.1"
          d="M210,125 C240,123 265,133 275,155 C285,173 280,197 265,209 C245,225 217,225 200,213 C180,200 173,175 183,155 C190,140 200,127 210,125 Z" />
      ...
      <g className="mc-cell mc-cell-2">
        <path fill={`url(#${gBody})`} stroke="#7BC9E8" strokeOpacity="0.22" strokeWidth="1.1"
          d="M65,300 C95,298 120,310 128,332 C138,355 130,375 112,386 C90,400 65,398 48,384 C32,370 25,348 35,330 C40,315 55,302 65,300 Z" />
      ...
      <g className="mc-cell mc-cell-3">
        <path fill={`url(#${gBody})`} stroke="#9CD9F0" strokeOpacity="0.32" strokeWidth="1.4"
          d="M126,115 C166,110 203,122 219,150 C233,168 239,192 227,212 C241,228 241,255 221,268 C227,288 209,310 185,312 C186,332 163,345 139,338 C123,350 99,350 85,338 C59,345 39,332 39,310 C19,308 6,288  15,268 C1,255  -1,230 15,215 C6,195  13,172 31,162 C45,140 71,122 101,118 C109,115 117,114 126,115 Z" />

        {/* Speckled cytoplasm */}
        <g fill="#1E5A8A">
          {speckles.map((s, i) => <circle key={i} cx={s[0]} cy={s[1]} r={s[2]} opacity={s[3]} />)}
        </g>
        <g fill="#B7E2F2">
          {speckles.slice(0, 11).map((s, i) => <circle key={i} cx={s[0] + 2} cy={s[1] - 1.5} r={s[2] * 0.5} opacity={0.4} />)}
        </g>

        {/* Kidney-bean nucleus */}
        <path fill={`url(#${gNuc})`} stroke="#0c2d4a" strokeOpacity="0.25" strokeWidth="1"
          d="M116,190 C96,190 83,208 83,228 C83,248 101,262 126,262 C144,262 159,255 167,238 C177,243 186,234 186,220 C188,203 174,186 157,182 C145,180 131,192 123,203 C121,195 118,190 116,190 Z" />
      </g>

      {/* === PARTICLES — fan diagonally to bottom-right (vessel area) === */}
      <g>
        {particles.map((p, i) => {
          // Calculation based on shared vector and individual multiplier
          const finalDx = BASE_DX * p.m;
          const finalDy = BASE_DY * p.m;
